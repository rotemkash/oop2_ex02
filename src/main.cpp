...
// As you will see below: many fields and many validators can be added to the
// form
...
// >> operator reads an int and << operator prints destination name if the value
// is valid and if not - the number.
...
int main() {
  ...
  auto yearOfBirthField =
      std::make_unique<Field<int>>("What is your year of birth?");

  auto destinationNames = ValuesToNames<DestinationNames>();
  auto destinationField =
      std::make_unique<Field<ValuesToNames<DestinationNames>>>(
          "What is your flight destination?\n" +
          destinationNames.valuesAndNames());

  auto flightTimes = ValuesToNames<FlightTimes>();
  auto flightTimeField = std::make_unique<Field<ValuesToNames<FlightTimes>>>(
      "What is your desired flight time range?\n" +
      flightTimes.valuesAndNames());
  auto wifiBundleNames = ValuesToNames<WifiBundle>();
  auto wifiBundleField = std::make_unique<Field<ValuesToNames<WifiBundle>>>(
  ...
  auto ageValidator = std::make_unique<RangeValidator<int>>(
      currentYear() - MAX_AGE, currentYear() - MIN_AGE);
  auto destinationValidator =
      std::make_unique<RangeValidator<ValuesToNames<DestinationNames>>>(1, 5);
  auto classValidator =
      std::make_unique<RangeValidator<ValuesToNames<FlightTimes>>>(1, 3);
  auto wifiBundleValidator =
      std::make_unique<RangeValidator<ValuesToNames<WifiBundle>>>(1, 3);
  ...
  auto destinationToClassValidator = std::make_unique<
      DestinationToFlightTimeValidator<Field<ValuesToNames<DestinationNames>>,
                                       Field<ValuesToNames<FlightTimes>>>>(
      destinationField.get(), flightTimeField.get());
  ...
  Field<ValuesToNames<WifiBundle>>>>(destinationField.get(),
  wifiBundleField.get());
  ...
  // Validation loop
  for (auto formIsCorrect = myForm.validateForm(); !formIsCorrect;
